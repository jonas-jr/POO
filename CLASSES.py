...
class Usuario:
    def __init__(self, nome, email, telefone,  rgcpf, genero, usuario, senha) : #cadastrado
        ...
        self.__senha = senha
class aluno (Usuario):
 def __init__(self,nome, email, telefone,  rgcpf, genero, usuario, senha, status, matrícula, notas) :
  ...
  self.__matrícula = matrícula#self.matrícula = pois esse atributo é novo, existe somente na classe aluno
  self.__notas = notas#self.notas = pois esse atributo é novo, existe somente na classe aluno
  def exibe(self):
      print(self.nome, self.email, self.telefone, self.rgcpf, self.genero,self.usuario, self.senha,self.status,self.matrícula,self.notas)
